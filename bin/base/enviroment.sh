...
for depen in ${depencies_to_delete[@]}; do
    echo "deleting ${BASEDIR}/${FOLDER_CODE}/${depen}"
    rm -rf ${BASEDIR}/${FOLDER_CODE}/${depen}
done
...
if [ "${FLAG_RHEEM}" = "true" ]; then
    ...
    FLAGS="${FLAGS} -Drheem.configuration=file://${BASEDIR}/${FOLDER_CONF}/${NAME_CONF_RHEEM} -Dspark.memory.fraction=0 "
fi
...
OTHER_FLAGS="-Dspring.main.web-application-type=none"
